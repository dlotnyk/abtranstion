    ...
    def import_fun(self,path_press):
        ...
        with open(path,'r') as f, open(path1,'r') as f1, open(path2,'r') as f2:
            ...
            for line, line1 in zip(f,f1):
                ...
                arr=np.array((d[2],d[13],d[6],d1[6],d[7],d1[7],press,0,d[10],d[15],d[16],d[17],d[18]),dtype=float)
                if a1<a:
                    try:
                        d2=next(f2).split()
                        mm1,dd1,yy1,hh1,mins1,ss1=self._gettime(d2[0],d2[1])
...
#-------------------------------------------------------------   
    ...
    def _forselect(self,tb_name,t1,t2):
        ...
        query="SELECT * FROM {} WHERE `date` >= '{}' AND `date` <= '{}' ORDER BY `date` ASC".format(tb_name,str(t1),str(t2))
        self.cursor.execute(query)
...
if __name__ == '__main__':    
    ...
    t1=datetime.datetime(2019,2,19,12,0,0,0)
    t2=datetime.datetime(2019,2,19,19,0,0,0)
    B=mysabdata(config, data_dir)
    B.table_name='dd5'
    B.qttime=B.dataset(q1=3,q2=4,temp=2,pressure=7,time=1)
#    B.dir_scan()
    time,Q1,Q2,temp,pres,date,dQ1,dQ2=B.select_interval(B.table_name,t1,t2)
    p1=np.mean(pres)
